    ...
    ajax.prototype.ajax = function(options, that) {
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (!ajax.callBack(msg, options, that)) return;
            }
        });
    };
        ...
        if (200 == ret.status) {
            ...
            if (record) {
                ...
                if (undefined != options.content) {
                    record[record['filed']] = options.content;
                }

                //以下的公共参数可以写在table格属性中
                if (!record['dataName']) {
                    record['dataName'] = that.parents('table').data('name');
                    record['tplid'] = that.parents('table').data('tplid');
                }

                if (!record['render']) {
                    record['render'] = that.parents('table').data('render');
                }


                var _data = layui.data(record['dataName']);

                if (_data['cacheData']) {
                    var list = _data['cacheData'].list;

                    if (record.key) {
                        var filed = record.key.split("=");
                    } else { return; }
                    for (var key in list) {
                ...
                if (record.render) {
                    if (options.dtable) {
                        options.dtable.setOption(record.tplid);
                        options.dtable.render(_data['cacheData']);
                    }
                }
