...
class cls_session
{
    ...
    function __construct(&$db, $session_table, $session_data_table, $session_name = 'AOS_ID', $session_id = '')
    {
        $this->cls_session($db, $session_table, $session_data_table, $session_name, $session_id);
    }

    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'AOS_ID', $session_id = '')
    {
    ...
    {
        $session = $this->db->getRow('SELECT userid, adminid, user_name, data, expiry FROM ' . $this->session_table . " WHERE sesskey = '" . $this->session_id . "'");
        if (empty($session))
        {
            $this->insert_session();
    ...
    function destroy_session()
    {
        ...
        /* AOSHOP 自定义执行部分 */
        if (!empty($GLOBALS['aos']))
        {
            $this->db->query('DELETE FROM ' . $GLOBALS['aos']->table('cart') . " WHERE session_id = '$this->session_id'");
        }
        /* AOSHOP 自定义执行部分 */
