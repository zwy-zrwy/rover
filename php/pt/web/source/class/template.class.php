...
//            }
                ...
                switch ($s[0])
                {
                    case 'escape':
                        ...
                        elseif ($s[1] == 'u8_url')
                        {
                            if (AO_CHARSET != 'utf-8')
                            {
                                $p = 'urlencode(aos_iconv("' . AO_CHARSET . '", "utf-8",' . $p . '))';
                            }
                            else
                            {
                                $p = 'urlencode(' . $p . ')';
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir   = 'template/' . $GLOBALS['_CFG']['template'] . '/'; // 模板所在路径
