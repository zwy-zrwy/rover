...
function sub_str($str, $length = 0, $append = true)
{
    ...
    {
        $newstr = mb_substr($str, 0, $length, AO_CHARSET);
    }
    elseif (function_exists('iconv_substr'))
    {
        $newstr = iconv_substr($str, 0, $length, AO_CHARSET);
    }
    else
    {
 ...
 * @return  void
 **/
function aos_header($string, $replace = true, $http_response_code = 0)
{
...
function aos_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
    ...
    if ($chs === NULL)
    {
        require_once(ROOT_PATH . 'source/class/iconv.class.php');
...
function json_str_iconv($str)
{
    if (AO_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return addslashes(stripslashes(aos_iconv('utf-8', AO_CHARSET, $str)));
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (AO_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return aos_iconv(AO_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
