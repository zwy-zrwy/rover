...
$cache_id = sprintf('%X', crc32('share_goods-'.$goods_id));
if (!$smarty->is_cached('share_goods.htm', $cache_id))
{
  ...
  else
  {
    ...
    $share['title'] = $goods['goods_name'];
    $share['desc'] = $goods['goods_brief'];
...
$smarty->display('share_goods.htm',$cache_id);
...
function get_goods_comment($goods_id)
{
    ...
    $sql = 'SELECT c.*,u.nickname,u.headimgurl,og.goods_attr FROM ' . $GLOBALS['aos']->table('comment') ." as c left join "
