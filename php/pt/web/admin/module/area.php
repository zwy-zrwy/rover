...
$exc = new exchange($aos->table('region'), $db, 'region_id', 'region_name');
/* act操作项的初始化 */
if ($operation == 'index' || $operation == 'area_manage')
{
    $operation = 'list';
}
...
if ($operation == 'list')
{
    ...
    if ($region_id > 0)
    {
        ...
        $action_link = array('text' => '返回上一级', 'href' => 'area_manage.php?act=list&&pid=' . $parent_id);
    }
    else
    {
        $action_link = '';
...
elseif ($operation == 'add_area')
{
    ...
    {
        make_json_error('区域名称不能为空！');
    }

    /* 查看区域是否重复 */
    if (!$exc->is_only('region_name', $region_name, 0, "parent_id = '$parent_id'"))
    {
        make_json_error('已经有相同的地区名称存在!');
    }

    $sql = "INSERT INTO " . $aos->table('region') . " (parent_id, region_name, region_type) ".
...
elseif ($operation == 'edit_area_name')
{
...
//-- 删除区域
/*------------------------------------------------------ */
elseif ($operation == 'drop_area')
{

    $id = intval($_REQUEST['id']);

    $sql = "SELECT * FROM " . $aos->table('region') . " WHERE region_id = '$id'";
    ...
    $sql="DELETE FROM ". $aos->table("region")."WHERE region_id".db_create_in($delete_region);
     $db->query($sql);
    if ($exc->drop($id))
    {
...
function new_region_id($region_id)
{
    ...
    $sql="SELECT region_id FROM ". $GLOBALS['aos']->table("region")."WHERE parent_id ".db_create_in($region_id);
