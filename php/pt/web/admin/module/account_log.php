...
elseif ($operation == 'insert' || $operation == 'update')
{
    ...
    /* 保存 */
    log_account_change($user_id, $user_money, $frozen_money, $rank_points, $pay_points, $change_desc, 2);

    /* 提示信息 */
    $links = array(
        array('href' => 'index.php?act=account_log&op=list&user_id=' . $user_id, 'text' => "返回列表")
    );
...
function get_accountlist($user_id, $account_type = '')
{
    ...
    /* 查询记录总数，计算分页数 */
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['aos']->table('account_log') . $where;
